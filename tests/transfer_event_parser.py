...
mock_event = {
    "args": {
        "from": "0xA8502b841D9fcD5d2a4C7439A13e87796AEd56A1",
        "to": "0x0001FB050Fe7312791bF6475b96569D83F695C9f",
        "value": 174003773318124380,
    },
    "event": "Transfer",
    "logIndex": 44,
    "transactionIndex": 40,
    "transactionHash": "0x69dd5baf185d9c4349d70a84963885ed301614d3c8d0457955d4df06e8bcbcb8",
    "address": "0x0bc529c00C6401aEF6D220BE8C6Ea1667F6Ad93e",
    "blockHash": "0x858a94be7abaa2aa124a3aef22bd4355afd460164b63b400ea12af7a3128e90f",
    "blockNumber": 10478438,
}


@pytest.fixture
def addresses():
    fname = os.path.join(os.path.dirname(__file__), "test_data/dummyAddresses.json")
...
@pytest.fixture
def events():
    fname = os.path.join(os.path.dirname(__file__), "test_data/dummyEvents.json")
    with open(fname) as f:
...
def test_handle_transfer_event(addresses):
    ...
    assert (
        transfer_event_parser.balances["0x0001FB050Fe7312791bF6475b96569D83F695C9f"][
            10478438
        ]
        == 174003773318124380
    )
...
def test_execute_events(addresses, events):
    ...
    assert (
        ...
        == 9995796750644777900207
    )
    assert (
        transfer_event_parser.balances["0x28b88cfD875C883cDb61938C97B8d1baabf31c88"][
            10478826
        ]
        == 90896908769421890
    )
...
def test_write_balances(dummy_addresses, dummy_balances, tmp_path):
    ...
    assert balances == [
        {"blockNumber": 2, "balance": 10},
        {"blockNumber": 3, "balance": 11},
        {"blockNumber": 4, "balance": 11},
        {"blockNumber": 5, "balance": 18},
    ]
